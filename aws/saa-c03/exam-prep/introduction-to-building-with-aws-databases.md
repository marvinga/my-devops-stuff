### Introduction to Building with AWS Databases

- WAF
- *dont confuse pillars & principles*

- pillars
  #### 1 - operational excellence
    - principles
      - perform operations as code
      - make frequent, small, reversible changes
      - refine operations procedures frequently
      - anticipate failure 
      - learn from operational failures
  
    - designing database solutions for operational excellence
      - cloudwatch
      - cloudwatch contributor insights for amazon dynamodb
      - dynamodb integrates with cloudwatch 

  #### 2 - reliability
      - resiliency or reliability  | disaster recovery
    - principles
      - automatically recover from failures - *dont confuse with anticipate failures*
      - test recovery procedures 
      - scale horizontally to increase aggregate workload availability
      - stop guessing capacity
      - manage change in automation

  #### 3 - performance efficiency
    - principles
      - democratized advanced technologies
      - go global in minutes
      - use serverless architecture
      - experiment more often - *dont confuse with make frequent, small, reversible changes*
      - consider mechanical sympathy

  #### 4 - cost optimization
    - principles
      - implement cloud financial management - *dont confuse with stop guessing capacity*
      - adopt a consumption model
      - measure overall efficiency
      - stop spending money on undifferentiated heavy lifting - *dont confuse with use serverless architecture*
      - analyze and attribute expenditure

  
  #### 5 - sustainability
    - principles
      - understand your impact
      - stablish sustainability goals
      - maximize utilization - *dont confuse with cost optimization*
      - anticipate and adopt new, more efficient hardware and software offerings
      - use managed services - *dont confuse use serverless architecture*
      - reduce the downstream impact of your cloud workloads
     
   #### 6 - security
    - principles
      - 
  
    
